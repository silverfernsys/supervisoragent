import mock, os, subprocess, unittest 
...
class ProcStatTest(unittest.TestCase):
    def setUp(self):
        ...
        self.resources =  os.path.join(os.path.abspath(os.path.dirname(__file__)), 'resources')
    def tearDown(self):
        self.cpu = None
        self.mem = None
    ...
    def test_cpu(self, mock_popen):
    	proc_id_stat = open(os.path.join(self.resources, 'proc_id_stat.txt')).read()
    ...
    def test_mem(self, mock_popen):
    	proc_id_smaps = open(os.path.join(self.resources, 'proc_id_smaps.txt')).read()
    	proc_meminfo = open(os.path.join(self.resources, 'proc_meminfo.txt')).read()
    	mock_popen.return_value.returncode.side_effect = [0, 0, 0, 0]
